...
static TestScene* CreateTestScene(int nIdx) {
	...
	switch (nIdx) {
	case TEST_TOUCH_SCALE:
		pScene = new TouchScaleTestScene();
		break;
	case TEST_TOUCH_EVENT:
		pScene = new TouchEventTestScene();
		break;
	case TEST_RETAIN_PROPERTY:
		pScene = new PropertyTestScene();
		break;
	default:
		break;
	}
...
TestController::TestController() :
	...
	// add close menu
	CCMenuItemImage *pCloseItem = CCMenuItemImage::create(s_pClose, s_pClose,
			this, menu_selector(TestController::closeCallback));
	CCMenu* pMenu = CCMenu::create(pCloseItem, NULL);

	pMenu->setPosition(CCPointZero);
	pCloseItem->setPosition(
			ccp( VisibleRect::right().x - 40, 40));
...
// #endif        
		...
		pMenuItem->setPosition(
				ccp( VisibleRect::center().x, (VisibleRect::top().y - (i + 1) * LINE_SPACE) ));
			}

	m_pItemMenu->setContentSize(
			CCSizeMake(VisibleRect::getVisibleRect().size.width, (TESTS_COUNT + 1) * (LINE_SPACE)));
...
void TestController::ccTouchesMoved(CCSet *pTouches, CCEvent *pEvent) {
	...
	if (nextPos.y
			> ((TESTS_COUNT + 1) * LINE_SPACE
					- VisibleRect::getVisibleRect().size.height)) {
		m_pItemMenu->setPosition(
				ccp(0, ((TESTS_COUNT + 1)* LINE_SPACE - VisibleRect::getVisibleRect().size.height)));
				return;
			}
